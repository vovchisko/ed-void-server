<template>
    <div class="ui input-search" v-bind:class="list.length || focused ?'z-overall':''">
        ...
        <button v-if="e_id" class="link b-remove" v-on:click="select_system(null)"><i class="i-cross"></i> clear</button>
        <div class="list edfx" v-if="list.length">
            <button class="link" v-for="b in list" v-on:click="select_system(b._id)">
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            select_system: function (system_id = null, external = false) {
                this.e_id = system_id;
